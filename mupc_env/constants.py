...
K_DROOP_MIN = 0.0                # 下垂系数下限 (kW/V), v3.1 对齐下游 [0,30]
K_DROOP_MAX = 30.0               # 下垂系数上限 (kW/V), v3.1 对齐下游 [0,30]
K_DROOP_RAMP_LIMIT = 10.0        # 下垂系数变化率限制 (kW/V/步)，ACT-02 对齐下游 PRD §6.5
...
CONTRACT_DEMAND_KW = 200.0       # 合同需量 (kW), v2.17 对齐下游
...
DEFAULT_WEIGHTS: dict[str, list[float]] = {
    ...
    # w6(电压斜率), w7(下垂平滑=0.5), w8(安全覆盖), w9(冲击预备度)
...
NORM_DISPATCH = (-TRANSFORMER_KVA, TRANSFORMER_KVA)  # 调度有功指令: [-200, 200] kW
NORM_DISPATCH_Q = (-Q_BATT_MAX_KVAR, Q_BATT_MAX_KVAR)  # 调度无功指令: [-300, 300] kVar (v2.14)
NORM_QUANTILE_LOAD = (0.0, LOAD_PEAK_KW)     # 分位数负荷预测: [0, 60] kW (v2.14 D10)
NORM_SHOCK_PROBABILITY = (0.0, 1.0)         # 冲击负荷概率: [0, 1] (v2.14 D10)
NORM_BASE_LOAD = (0.0, LOAD_PEAK_KW)         # 基荷: [0, 60] kW (v2.14 D10)
NORM_QUANTILES = 15                          # 分位数预测步数 (v2.14 D10)

# ═══════════════════════════════════════════════════════════════
# D10 冲击负荷预备度奖励参数 (v2.17 对齐下游 v2.13)
# ═══════════════════════════════════════════════════════════════

SHOCK_THRESHOLD_KW = 10.0          # P90 - P50 冲击检测阈值 (kW)
SOC_RESERVE_TARGET = 0.7           # 目标 SOC 缓冲 (冲击预备)
P_REF_RESERVE_TARGET = 10.0        # 目标放电裕度 (kW)
SHOCK_READINESS_W_SOC = 20.0       # SOC 预备度权重 w1
SHOCK_READINESS_W_P = 10.0         # 有功预备度权重 w2
