...
# TOU 电价表（夏季：6-9 月）
TOU_SCHEDULE_SUMMER = [
    ( 0,  8, 0, 0.40),  # 谷：23:00-08:00
    ( 8, 10, 1, 0.85),  # 平：08:00-10:00
    (10, 12, 2, 1.20),  # 峰：10:00-12:00
    (12, 14, 3, 1.50),  # 尖峰：12:00-14:00（夏季高峰）
    (14, 17, 1, 0.85),  # 平：14:00-17:00
    (17, 21, 2, 1.35),  # 峰：17:00-21:00（晚高峰）
    (21, 23, 1, 0.85),  # 平：21:00-23:00
    (23, 24, 0, 0.40),  # 谷：23:00-24:00
]

# TOU 电价表（冬季/过渡季：10-5 月）
TOU_SCHEDULE_WINTER = [
    ( 0,  8, 0, 0.42),  # 谷：23:00-08:00（冬季电价略高）
    ( 8, 10, 1, 0.82),  # 平：08:00-10:00
    (10, 12, 2, 1.10),  # 峰：10:00-12:00
    (12, 14, 2, 1.10),  # 峰：12:00-14:00（冬季无尖峰）
    (14, 17, 1, 0.82),  # 平：14:00-17:00
    (17, 21, 2, 1.25),  # 峰：17:00-21:00（晚高峰）
    (21, 23, 1, 0.82),  # 平：21:00-23:00
    (23, 24, 0, 0.42),  # 谷：23:00-24:00
]
...
def _tou_for_hour(hour: int, month: int) -> tuple[int, float]:
    """返回单一时段的 (tariff_id, price)，夏季/冬季分别定价。"""
    schedule = TOU_SCHEDULE_SUMMER if month in (6, 7, 8, 9) else TOU_SCHEDULE_WINTER
    for start, end, tid, price in schedule:
        if start <= hour < end:
            return tid, price
    return 0, 0.42  # 兜底
