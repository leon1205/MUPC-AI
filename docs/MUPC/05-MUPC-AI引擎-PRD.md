...
| A2 | load_shedding | f64 | [0.0, 500.0] | kW | 可中断负荷切除量 |
| A3 | pv_limit | f64 | [0.0, 1.0] | - | 光伏限功率比例（0.0=完全限功率，1.0=不限功率，v2.6 恢复）|

> **v2.6 变更说明：** 动作空间从 2 维扩展为 3 维，恢复 pv_limit 作为主动弃光手段。Q_batt 由实时电压调节器闭环控制，不经过 RL 动作空间。
>
...
> **v2.6 动作空间说明：** 动作空间从 2 维扩展为 3 维（P_batt + Load_shedding + pv_limit），pv_limit 恢复为 AI 可控维度，用于主动弃光。Q_batt 控制完全交由实时控制核心根据电压闭环调节。
