...
| 工具 | 职责 | 不做什么 |
|------|------|----------|
| **AI Policy (PyTorch)** | 学习"看到状态 → 输出动作" | 不知道任何物理 |
...
### 8.5.4 奖励的细节: 13 个子奖励如何工作?

回到我们的 `_reward_agri()` (MODE-01), 实际有 13 个子奖励项:

| 编号 | 奖励项 | 目的 | 公式简化 |
|------|--------|------|----------|
| w1 | 光伏消纳 | 鼓励多用光伏 | `min(光伏自用/总光伏, 1.0)` |
| w2 | 电池损耗 | 鼓励减小 C-rate | `-C_rate² × α` |
| w3 | 过载惩罚 | 保护变压器 | 3 段惩罚 |
| w4 | P-Q 协同 | 电压感知响应 | Sigmoid 平滑 |
| w5 | 变化率 | 减小突变 | `-ΔP / 100` |
| w6 | 电压斜率 | 平滑电压变化 | `-ΔV × (1+2×|ΔV|)` |
| w7 | 下垂平滑 | 减小 k_droop 变化 | `-|Δk_droop| - 超限惩罚` |
| w8 | 安全覆盖 | 配合下游 SafetyOverride | 分层 (consecutive<10 vs ≥10) |
| w9 | 过载预警 | 提前避免过载 | `-0.85 < lr < 1.0 时平滑惩罚` |
| w10 | SOC 预警 | 避免接近 SOC 边界 | `soc_margin < 0.1` |
| w11 | SOC 平衡 | 鼓励 SOC=0.5 | `-|soc-0.5|` |
| w12 | 冲击预备度 | 应对冲击负荷 | `SOC 与 P_ref 余量` |
| w13 | 状态改善率 | 鼓励朝好的方向走 | `Δv_dev 改善` |
...
| `mupc_env/observation.py` | 观测构建 (63 维) |
