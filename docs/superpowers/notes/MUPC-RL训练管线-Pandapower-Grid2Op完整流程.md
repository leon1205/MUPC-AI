...
│     输入: 78 维观测 (v2.14 单模式) / 79 维 (多模式)          │
│     输出: 2 维动作 [p_ref, k_droop]                          │
└───────────────────────────┬──────────────────────────────────┘
                            │ 2D 动作 (归一化 [-1, 1])
                            ↓
┌──────────────────────────────────────────────────────────────┐
│  MupcEnv  (mupc_env/core.py)                                │
│  • 动作校验 (ACT-01~05, v2.17)                                │
│  • 动作反归一化 (k_droop ∈ [-100, 100] kW/V, v2.17)          │
│  • 调度 Grid2Op / Pandapower 物理仿真                        │
│  • 计算奖励 (SCENE-01: 9 项归一化 + 3 项未归一化附加)        │
│  • 构建 78 维观测 (D1-D10)                                    │
└───────────────────────────┬──────────────────────────────────┘
                            │ 调用 (可选, 默认降级到 VoltageSimulator)
                            ↓
┌──────────────────────────────────────────────────────────────┐
│  Grid2Op  (Python 库, 电网仿真框架)                          │
│  • 协调时间推进 (episode/step 管理)                           │
│  • 数据注入 (NumpyChronics 适配三相矩阵)                     │
│  • 状态机管理 (reset/step API)                               │
│  • 借用底层 Backend 跑潮流                                   │
└───────────────────────────┬──────────────────────────────────┘
                            │ 调度
                            ↓
...
│  • 定义电网拓扑 (线路/变压器/光伏/电池/负荷)                  │
│  • 跑 Newton-Raphson 潮流计算 (P-U-Q-θ)                      │
│  • 返回每个节点的电压 U, 每个支路的功率 P                      │
└──────────────────────────────────────────────────────────────┘
                            ↓
                    返回: va, vb, vc (三相电压)
```
...
| 工具 | 职责 | 不做什么 |
|------|------|----------|
| **AI Policy** | 学习"看到状态 → 输出动作" | 不知道任何物理 |
| **MupcEnv** | 编排训练循环, 校验动作, 计算奖励, 构建观测 | 不做潮流计算, 不跑模型 |
| **Grid2Op** | 协调时间、注入数据、调度 Backend | 不做具体物理计算 |
| **Pandapower** | 定义网络、跑潮流 (物理计算) | 不管时间、不管 AI |

> **MupcEnv 是中间编排层**, 在 v2.14 之前, 三大工具中没有 MupcEnv 直接参与流程图, 实际代码中 MupcEnv 才是真正调用方。引入后职责更清晰: AI 不感知物理仿真细节, Pandapower 不感知时间与 AI 动作。
...
| 接口 | 上游 (AI 输出) | MupcEnv 转换 | 下游 (仿真器) |
|------|----------------|--------------|----------------|
| 观测 | 78/79 维 ndarray | 归一化 → 反归一化 | Grid2Op/Pandapower 物理量 |
| 动作 | 2 维 ndarray ∈ [-1, 1] | 反归一化: `p_ref = a[0] × 50` (kW), `k_droop = a[1] × 100` (kW/V) | net.storage[0].p_mw = ... |
| 电压 | - | - | va/vb/vc (p.u., 0.85~1.15) |
| 奖励 | - | 9 项归一化 + 3 项未归一化附加 (v2.17) | - |
